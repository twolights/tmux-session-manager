...
- `terminal-notifier` is installed (`require_cmd terminal-notifier`). If absent, print a one-line install hint (`brew install terminal-notifier`) and exit 1.
...
After the settings.json write, invoke `terminal-notifier -title 'tms' -message 'Notification hook installed. You should see this banner.'` and then print, unconditionally, the following block to stdout:
...
Rationale: `terminal-notifier` does not expose a reliable permission-denied exit code on current macOS (banner suppression typically produces exit 0), so runtime detection is unreliable. Catching the most common failure mode (permissions never granted) proactively at install time, plus printing the exact System Settings path in the install output, covers the user-diagnosability gap without any runtime complexity.
...
- Does NOT manage `terminal-notifier` installation; that's the user's responsibility, surfaced via error messages.
